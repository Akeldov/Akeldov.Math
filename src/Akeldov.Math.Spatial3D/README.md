# Akeldov.Math.Spatial3D

Akeldov.Math.Spatial3D is a .NET library for three-dimensional geometry and vector math.

## Features

- Immutable single-precision `VectorXYZ` and integer `VectorXYZInt` values.
- Immutable `PointXYZ` values with distance measurement, vector translation, and linear interpolation.
- Centroid and nearest-item helpers for collections of `IHasPosition3D` values.
- Signed and unsigned point-distance provider contracts.
- Geometric and parameterized infinite lines, directed rays, geometric and parameterized finite segments and segment chains, quadratic and cubic Bezier paths, and curve/path contracts for projection, distance measurement, finite length, endpoints, and parameterization.
- Surface projection and distance contracts, with an infinite plane primitive.
- Vector length, normalization, dot product, cross product, and arithmetic operations.
