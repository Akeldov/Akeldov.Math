...
{
    /// <summary>
    /// Represents a finite directed non-uniform rational B-spline (NURBS) curve in three-dimensional space.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Supports positive weights, degree one or higher, and clamped or unclamped knot vectors. Interior
    /// knot multiplicity cannot exceed the degree, so the path remains continuous.
    /// </para>
    /// <para>
    /// <see cref="GetPointAt"/> and <see cref="GetPointAtKnot"/> evaluate the rational spline using
    /// de Boor's algorithm in homogeneous coordinates. Length, distance, projection, and length-coordinate
    /// traversal use a cached polyline with <see cref="SegmentsPerKnotSpan"/> equal parameter subdivisions
    /// of each non-empty knot span. This is an approximation, not an error bound; increase the subdivision
    /// count for sharp bends or extreme weights.
    /// </para>
    /// </remarks>
    public sealed class Nurbs : IFinitePath
    {
        ...
        /// The degree, a point, weight, knot, or subdivision count is invalid, or the approximate length cannot be
            ...
            int segmentsPerKnotSpan = 64)
        {
            ...
            for (int i = 0; i < _controlPoints.Length; i++)
            {
                ...
                if (float.IsNaN(point.X) || float.IsInfinity(point.X) ||
                    ...
                    throw new ArgumentOutOfRangeException(nameof(controlPoints), "Control point coordinates must be finite.");
        ...
        /// <summary>
        /// Gets a read-only structural view of the copied weight state.
        /// </summary>
        ...
        /// <summary>
        /// Evaluates the rational spline at a normalized parameter.
        /// </summary>
        /// <param name="t">
        ...
        /// <summary>
        /// Evaluates the rational spline at a parameter in the original knot units.
        /// </summary>
        /// <param name="knot">
