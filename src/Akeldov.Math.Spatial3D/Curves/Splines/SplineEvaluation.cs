...
#pragma warning restore IDE0130
{
    ...
    {
        public static float[] CopyAndValidateKnots(
            ...
            for (int i = 1; i < copy.Length; i++)
            {
                ...
                if (multiplicity > (interior ? degree : degree + 1))
                    throw new ArgumentException(
                        "Knot multiplicity exceeds the continuous path limit.",
                        nameof(knots));
            }
        ...
        public static PointXYZ[] CreateApproximation(
            int degree,
            PointXYZ[] controlPoints,
            float[]? weights,
            float[] knots,
            int segmentsPerKnotSpan)
        {
            ...
            if (pointCount > int.MaxValue)
                throw new ArgumentOutOfRangeException(
                    ...
                    "The approximation contains too many points.");

            var points = new PointXYZ[(int)pointCount];
            int workLength = (degree + 1) * (weights is null ? 3 : 4);
        ...
        public static PointXYZ Evaluate(
            int degree,
            PointXYZ[] controlPoints,
            float[]? weights,
            float[] knots,
        ...
        private static PointXYZ EvaluateSpan(
            int degree,
            PointXYZ[] controlPoints,
            float[]? weights,
            float[] knots,
            ...
            Span<double> work)
        {
            // Polynomial splines use XYZ; rational splines use homogeneous (w*x, w*y, w*z, w).
            // Both use the same de Boor recurrence, with double intermediates.
            int dimension = weights is null ? 3 : 4;
            for (int j = 0; j <= degree; j++)
            {
                int index = span - degree + j;
                int offset = j * dimension;
                PointXYZ point = controlPoints[index];
                if (weights is null)
                {
                    work[offset] = point.X;
                    work[offset + 1] = point.Y;
                    work[offset + 2] = point.Z;
                }
                else
                {
                    ...
                    work[offset + 2] = point.Z * weight;
                    work[offset + 3] = weight;
                }
            }
            ...
            for (int level = 1; level <= degree; level++)
            {
                for (int j = degree; j >= level; j--)
                {
                    ...
                    for (int coordinate = 0; coordinate < dimension; coordinate++)
                    {
                        ...
                        work[current] =
                            (1.0 - amount) * work[current - dimension] +
                            amount * work[current];
                    }
                }
            }
            ...
            {
                return new PointXYZ(
                    (float)work[resultOffset],
                    (float)work[resultOffset + 1],
                    (float)work[resultOffset + 2]);
            }

            double resultWeight = work[resultOffset + 3];
            return new PointXYZ(
                (float)(work[resultOffset] / resultWeight),
                (float)(work[resultOffset + 1] / resultWeight),
                (float)(work[resultOffset + 2] / resultWeight));
        }
    }
}
