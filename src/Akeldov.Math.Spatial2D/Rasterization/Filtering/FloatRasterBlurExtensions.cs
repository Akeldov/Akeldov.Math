...
namespace Akeldov.Math.Spatial2D.Rasterization
{
    ...
    public static class FloatRasterBlurExtensions
    {
        ...
        public static Raster<float> GaussianBlur(this IRaster<float> raster, float sigma)
        {
            ...
            return new Raster<float>(resolution, CreateGaussianBlurValues(raster, sigma, radius));
        }
        ...
        /// <summary>
        /// Applies a Gaussian blur while preserving the source raster geometry, using a kernel
        ...
        /// </param>
        /// <returns>
        /// A new mutable spatial raster owned by the caller. Its geometry is the source geometry,
        /// and the source raster is not modified. At raster boundaries, weights are normalized
        /// over the source cells that are present.
        /// </returns>
        /// <exception cref="ArgumentNullException">
        /// Thrown when <paramref name="raster"/> is <see langword="null"/>.
        /// </exception>
        ...
        /// Thrown when <paramref name="sigma"/> is not finite and positive.
        /// </exception>
        public static SpatialRaster<float> GaussianBlur(this ISpatialRaster<float> raster, float sigma)
        {
            if (raster == null)
                throw new ArgumentNullException(nameof(raster));
            ...
            int maximumRadius = System.Math.Max(resolution.X - 1, resolution.Y - 1);
            int radius = (int)System.Math.Min(System.Math.Ceiling(3d * sigma), maximumRadius);
            return new SpatialRaster<float>(raster.Geometry, CreateGaussianBlurValues(raster, sigma, radius));
        }

        /// <summary>
        /// Applies a Gaussian blur while preserving the source raster geometry, using an
        /// explicitly truncated kernel.
        /// </summary>
        /// <param name="raster">The source spatial raster.</param>
        /// <param name="sigma">
        /// The positive finite Gaussian standard deviation, measured in raster cells.
        ...
        /// <paramref name="radius"/> is negative.
        /// </exception>
        public static SpatialRaster<float> GaussianBlur(
            this ISpatialRaster<float> raster,
            float sigma,
            int radius)
        {
            if (raster == null)
        ...
        private static float[] CreateGaussianBlurValues(IRaster<float> raster, float sigma, int radius)
        {
            VectorXYInt resolution = raster.Resolution;
