...
namespace Akeldov.Math.Spatial2D.Rasterization
{
    ...
    public static class IRasterExtensions
    {
        ...
        /// <summary>
        /// Maps each value of the specified raster and associates the result with the provided geometry.
        /// </summary>
        /// <typeparam name="TSource">The source raster value type.</typeparam>
        /// <typeparam name="TResult">The result raster value type.</typeparam>
        /// <param name="raster">The source raster.</param>
        /// <param name="geometry">The result raster geometry. Its resolution must match the source raster resolution.</param>
        /// <param name="selector">The function that maps each source value to a result value.</param>
        /// <returns>A new spatial raster whose value array is new, mutable, and owned by the caller.</returns>
        public static SpatialRaster<TResult> MapValues<TSource, TResult>(
            this IRaster<TSource> raster,
            ...
            Func<TSource, TResult> selector)
        {
            ...
            if (geometry.Resolution != raster.Resolution)
                throw new ArgumentException("Raster geometry resolution must match the source raster resolution.", nameof(geometry));
            ...
            return new SpatialRaster<TResult>(geometry, values);
        }
    }
}
