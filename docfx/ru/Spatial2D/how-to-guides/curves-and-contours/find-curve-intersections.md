...
## Сохранить пользовательский допуск в устаревшем коде

`GetPointIntersections(Ray)` использует стандартный геометрический допуск библиотеки, и для
нового кода рекомендуется этот API. Устаревшая перегрузка `GetRayIntersections(Ray, float)`
остаётся для старого кода, которому необходимо выбирать пользовательский допуск. Её аргумент
`geometryEpsilon` измеряется в единицах мировой системы координат и управляет сравнениями около
точек касания и концов, а также для коллинеарных наложений и почти параллельных кривых. Например,
увеличенный допуск позволяет считать отрезок рядом с лучом коллинеарным ему:
...
#pragma warning disable CS0618 // Устаревший API необходим для пользовательского допуска.
...
Предпочитайте `GetPointIntersections`, если не требуется сохранить сценарий с пользовательским
допуском. Если без устаревшей перегрузки не обойтись, увеличивайте допуск только с учётом
масштаба и ожидаемого численного шума исходных данных. Слишком большое значение может объединить
близкие точки или считать близкие, но непересекающиеся объекты пересекающимися. Допуск должен
быть конечным и неотрицательным.
