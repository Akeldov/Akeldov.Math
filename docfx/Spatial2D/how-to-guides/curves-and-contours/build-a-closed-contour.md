...
Use `CompositeContour` to turn ordered vertices or a chain of finite paths into one closed
...
IReadOnlyList<IFinitePath> edges = contour.Curves;    // 4 segments
```
...
For a mixed boundary, build an `IReadOnlyList<IFinitePath>`. Each path's `EndPoint` must meet the
...
`ParameterizedSegmentChain`, and the Bezier path types all implement `IFinitePath` and can take
part in the chain.
...
distinct adjacent vertices. A path-based contour requires at least one non-null `IFinitePath`;
