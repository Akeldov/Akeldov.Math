...
```text
Stored influence sources
        |
        v
Optional culler selects a non-empty local subset
        |
        v
...
This separation lets the same source set use nearest-neighbor, inverse-distance, or barycentric
sampling, with or without geometric culling.
...
## Cull sources before sampling

A culler selects the sources relevant at each point before the sampler runs. Culling changes the
local interpolation neighborhood; it is not only a performance switch.

Built-in cullers operate on point influence sources:

| Culler | Selection behavior |
|---|---|
| <xref:Akeldov.Math.Spatial2D.Fields.HalfPlaneCuller`1> | Visits sources from nearest to farthest and excludes sources hidden behind half-plane boundaries introduced by nearer sources. |
| <xref:Akeldov.Math.Spatial2D.Fields.DelaunayCuller`1> | Returns the containing Delaunay triangle; outside the triangulation it returns the nearest convex-hull vertex or edge, with a collinear fallback. |

`DelaunayCuller` requires at least three sources with unique positions. Non-collinear sources are
triangulated when the culler is constructed. Both cullers always return a new mutable, non-empty
list owned by the caller.

Pass the same logical source set to the culler and field:
...
Custom implementations of <xref:Akeldov.Math.Spatial2D.Fields.IInfluenceSourceCuller`1> must
return at least one source. Returning `null` or an empty list makes the field fail explicitly
rather than silently sampling an undefined neighborhood.
...
Influence fields require a non-empty source collection with no `null` elements. They copy source
references into private storage and expose the retained structure through the read-only
`InfluenceSources` property. Later mutations of the caller's list or array do not change the
field's order or source count.
