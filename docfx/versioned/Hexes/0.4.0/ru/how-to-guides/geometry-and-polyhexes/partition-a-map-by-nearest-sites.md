...
Передайте логическую карту с той же топологией, чтобы разбить только выбранные центры:
...
Участвующие гексы получают ближайшую взвешенную ячейку. Исключённые гексы возвращают `null`, для
них `Participates` возвращает `false`, и они не входят в `HexIndexes` ни одной ячейки. Результат
копирует маску, поэтому последующие изменения исходной карты на него не влияют. Используйте
`ToMutableParticipationMask()` или `ToMutableHexMap()`, когда нужны независимые изменяемые копии.
