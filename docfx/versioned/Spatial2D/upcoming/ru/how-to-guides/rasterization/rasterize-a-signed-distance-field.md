...
замкнутую цепочку требует `CompositeContour`. Каждый `IContourPath` также предоставляет запросы
подсчёта пересечений для правила заполнения и пересечения с лучом, используемые составным
контуром.
