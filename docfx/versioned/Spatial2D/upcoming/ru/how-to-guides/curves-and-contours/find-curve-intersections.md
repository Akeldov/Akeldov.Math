...
начале луча или дальше по его направлению. Интерфейсы кривых, включая `ICurve`, `IContourPath` и
`IContour`, пересечения с лучом не объявляют.
...
## Сохранить поддерживаемый конкретный тип

Пересечение с лучом — бинарная операция, предоставляемая методами расширения, а не возможность
интерфейса кривой. Сохраняйте поддерживаемый конкретный тип геометрии, чтобы компилятор мог
выбрать его перегрузку. Поддерживаемые несоставные типы контуров также предоставляют конкретные
перегрузки. У `CompositeContour` и `ParameterizedCompositeContour` их нет: разнородные части
хранятся через `IContourPath`, у которого нет общей операции бинарного пересечения.
...
`ParameterizedSegmentChain` объединяет результаты своих отрезков и удаляет в точности равные
общие точки. Благодаря этому луч, проходящий через вершину цепочки, не возвращает одно и то же
положение отдельно для каждого соседнего отрезка.
