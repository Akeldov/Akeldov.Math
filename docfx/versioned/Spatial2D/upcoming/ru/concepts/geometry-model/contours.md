...
| <xref:Akeldov.Math.Spatial2D.Curves.IContourPath> | Конечный направленный путь с подсчётом пересечений для правила заполнения и объявленным напрямую запросом пересечения с лучом, необходимым составному контуру. |
...
`IContour` не предоставляет полиморфные пересечения с лучом; конкретные типы контуров выполняют
эти операции через методы расширения.
...
Конкретные типы контуров предоставляют `GetPointIntersections` через методы расширения для
пересечений; сам `IContour` эту операцию не объявляет. `IContourPath` напрямую объявляет
перегрузку для луча, необходимую для полиморфного вызова внутри составного контура. Составные
контуры объединяют результаты своих частей и удаляют повторные пересечения в общих концах по
точному равенству точек.
