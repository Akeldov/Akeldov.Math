...
| <xref:Akeldov.Math.Spatial2D.Curves.IContourPath> | Конечный направленный путь с возможностями подсчёта пересечений направленного вправо луча и пересечения с произвольным лучом, необходимыми составному контуру. |
...
<xref:Akeldov.Math.Spatial2D.Curves.IRightwardCrossingProvider> для запросов правил заполнения и
<xref:Akeldov.Math.Spatial2D.Curves.IRayIntersectionProvider> для полиморфных пересечений с лучом.
...
пересечений; для полиморфного вызова используйте `IRayIntersectionProvider`. Составные контуры
объединяют результаты своих частей и удаляют повторные пересечения в общих концах по точному
равенству точек.
