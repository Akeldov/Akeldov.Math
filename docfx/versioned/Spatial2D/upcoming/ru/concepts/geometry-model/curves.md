...
| <xref:Akeldov.Math.Spatial2D.Curves.IRayIntersectionProvider> | Возвращает изолированные точки пересечения с произвольным лучом. |
| <xref:Akeldov.Math.Spatial2D.Curves.IContourPath> | Объединяет `IFinitePath` с обоими пространственными запросами, необходимыми составным контурам. |
...
пересечения, расположенные в прямом направлении переданного луча. Для полиморфного вызова
используйте `IRayIntersectionProvider`:
