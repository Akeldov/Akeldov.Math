...
| Сплайн | <xref:Akeldov.Math.Spatial2D.Curves.BSpline> | Конечный путь; `[0, Length]` | Кривую определяют степень, управляющие точки и неравномерный вектор узлов. |
| Сплайн | <xref:Akeldov.Math.Spatial2D.Curves.Nurbs> | Конечный путь; `[0, Length]` | В дополнение к данным B-сплайна нужны рациональные веса. |
