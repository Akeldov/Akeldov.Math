...
Для барицентрической интерполяции
<xref:Akeldov.Math.Spatial2D.Fields.DelaunayInfluenceSourceIndex`1> владеет снимком источников и
перед каждым вычислением выбирает содержащий точку треугольник Делоне:
...
`DelaunayInfluenceSourceIndex` требует не менее трёх источников с уникальными положениями. Вне
области триангуляции он выбирает ближайшую вершину или ребро выпуклой оболочки. Передайте полю
`sources` напрямую вместо индекса, если семплер должен учитывать все источники.
...
включительным диапазоном от `field.Min` до `field.Max`, полученным из принадлежащего индексу
снимка источников.
