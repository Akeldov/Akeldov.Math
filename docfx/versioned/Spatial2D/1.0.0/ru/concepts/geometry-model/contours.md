...
Размеры прямоугольника могут быть нулевыми, но не отрицательными. Положительные ширина и высота
задают обычную область и границу; один нулевой размер задаёт отрезок; два нулевых размера — точку.
У всех шести структур прямоугольных областей и контуров корректное значение `default`, которое
представляет точку в начале координат.

Вырожденный прямоугольный контур замкнуто проходит отрезок туда и обратно, поэтому его `Length`
вдвое больше длины отрезка. У точечного контура `Length` равна нулю. У вырожденных
прямоугольников нет внутренней области: их `SignedDistance` равна нулю на представленном отрезке
или точке и положительна снаружи. Эти правила геометрии не меняют отдельную проверку аргументов,
которую выполняет `RasterGeometry`.
