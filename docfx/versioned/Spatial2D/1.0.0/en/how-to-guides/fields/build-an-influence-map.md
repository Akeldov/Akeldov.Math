...
For barycentric interpolation, a
<xref:Akeldov.Math.Spatial2D.Fields.DelaunayInfluenceSourceIndex`1> owns the source snapshot and
selects the containing Delaunay triangle before each sample is evaluated:
...
`DelaunayInfluenceSourceIndex` requires at least three sources with unique positions. Outside
the triangulated area it selects the nearest convex-hull vertex or edge. Pass `sources` directly
to the field instead of a source index when the sampler should evaluate every source.
...
inclusive `field.Min` and `field.Max` range derived from the index-owned source snapshot.
