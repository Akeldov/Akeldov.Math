...
2. An optional culler selects the local sources relevant to each sampled point.
...
| Piecewise-linear values across local source triangles | <xref:Akeldov.Math.Spatial2D.Fields.BarycentricFloatSampler`1>, usually with <xref:Akeldov.Math.Spatial2D.Fields.DelaunayCuller`1>. |

Culling changes the interpolation neighborhood as well as its cost. Use
<xref:Akeldov.Math.Spatial2D.Fields.DelaunayCuller`1> for triangle-based local interpolation or
<xref:Akeldov.Math.Spatial2D.Fields.HalfPlaneCuller`1> to exclude sources hidden behind nearer
half-plane boundaries.

## How-to guides

- [Build an influence map](build-an-influence-map.md) — create weighted point sources, choose a
  sampler, apply Delaunay culling, rasterize the field as a heat map, and save it as PNG.

## Related documentation

- [Fields concepts](../../concepts/fields.md) explains field interfaces, source ownership,
  sampling strategies, culling, and curve-based influence in depth.
