...
| <xref:Akeldov.Math.Spatial2D.Contours.ICompositeContour> | Exposes the finite paths forming a contour as a read-only structural view. |
...
All contours also implement <xref:Akeldov.Math.Spatial2D.Curves.ICurve>, so projection, ray
intersections, and rightward crossing counts work uniformly for curves and contours.
...
For a mixed boundary, pass an `IReadOnlyList<IFinitePath>`. Every path must end where the next
