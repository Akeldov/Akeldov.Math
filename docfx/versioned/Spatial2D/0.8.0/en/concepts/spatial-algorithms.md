...
| Select the local triangle or hull feature around a query point | Delaunay source culling | <xref:Akeldov.Math.Spatial2D.Fields.DelaunayCuller`1> |
| Exclude point sources hidden behind nearer half-plane boundaries | Half-plane source culling | <xref:Akeldov.Math.Spatial2D.Fields.HalfPlaneCuller`1> |

Poisson disk sampling creates new positions. Voronoi partitioning groups objects that already
have positions. Influence cullers return a temporary local subset for one field sample; they do
not permanently repartition the source collection.

<a id="generate-poisson-disk-points"></a>
<a id="vary-spacing-with-a-field"></a>

For point generation with constant and adaptive spacing, see
[Poisson Disk Sampler](samplers/poisson-disk.md) under [Samplers](samplers/index.md).
...
Influence cullers solve a different problem from Voronoi partitioning: for each query point, they
return the point sources that a sampler should consider.

`DelaunayCuller<TPointSource>` requires at least three sources with unique positions. It builds a
triangulation for non-collinear input. A query inside the hull receives the three sources of its
containing triangle; a query outside receives the nearest hull vertex or the two endpoints of the
nearest hull edge. Collinear inputs use an explicit fallback.

`HalfPlaneCuller<TPointSource>` accepts one or more sources. It processes them from nearest to
farthest and removes sources hidden behind perpendicular half-plane boundaries created by those
already selected.

Both cullers return a new mutable, non-empty list owned by the caller. They are designed to be
passed to point influence fields; see [Fields](fields.md) for the full sampling pipeline.
