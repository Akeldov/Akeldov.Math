...
| <xref:Akeldov.Math.Spatial2D.Contours.ICompositeContour> | Предоставляет конечные пути контура как структурное представление только для чтения. |
...
Все контуры также реализуют <xref:Akeldov.Math.Spatial2D.Curves.ICurve>, поэтому проекция,
пересечения с лучом и подсчёт пересечений направленного вправо луча единообразно работают для
кривых и контуров.
...
Для смешанной границы передайте `IReadOnlyList<IFinitePath>`. Каждый путь должен заканчиваться
...
var curvedBoundary = new CompositeContour(new IFinitePath[]
{
...
Необязательный параметр `geometryEpsilon` методов знакового расстояния и пересечения измеряется
в мировых единицах координат. Он управляет сравнениями вблизи границы и общих вершин.

Контуры наследуют `GetRayIntersections` от `ICurve`. Составные контуры объединяют результаты
своих частей и удаляют повторные пересечения в общих концах с учётом переданного допуска.
