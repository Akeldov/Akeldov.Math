...
## Combine segments, arcs, Bezier paths, and splines
...
`ParameterizedSegmentChain`, the Bezier path types, `BSpline`, and `Nurbs` all implement
`IContourPath`. That interface combines the finite directed path contract with fill-rule crossings; it does not
