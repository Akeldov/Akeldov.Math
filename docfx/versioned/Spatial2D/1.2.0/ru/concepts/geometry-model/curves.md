...
внутреннюю аппроксимацию ломаной для длины, проекции и расстояния. Пересечения кривых Безье,
B-сплайнов и NURBS решают исходные полиномиальные или рациональные уравнения без разбиения на
ломаную; `SegmentsPerKnotSpan` не влияет на бинарные пересечения.
