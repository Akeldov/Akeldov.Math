...
Необязательный параметр `geometryEpsilon` методов знакового расстояния измеряется в мировых
единицах координат. Он управляет сравнениями вблизи границы.

Контуры наследуют `GetPointIntersections` от `ICurve`. Составные контуры объединяют результаты
своих частей и удаляют повторные пересечения в общих концах со стандартным геометрическим
допуском библиотеки.
