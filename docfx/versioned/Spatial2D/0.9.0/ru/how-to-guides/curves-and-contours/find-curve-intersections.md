...
предоставляет перегрузки для целевых `QuadraticBezier` и `CubicBezier`.
...
Пересечения с целевой кривой Безье аналогично изолируют исходный результант степени до девяти
без линеаризации кривых. Результат упорядочивается от `StartPoint` к `EndPoint` целевой кривой.
