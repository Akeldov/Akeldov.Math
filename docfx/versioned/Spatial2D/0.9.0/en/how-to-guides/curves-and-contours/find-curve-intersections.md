...
Every listed linear, circular, quadratic, and cubic source type also provides overloads for target
`QuadraticBezier` and `CubicBezier` curves.
...
restrict results according to endpoint inclusion. A target `QuadraticBezier` or `CubicBezier`
orders intersections from its `StartPoint` to its `EndPoint`.
...
and round coordinates to `float` without a geometry-epsilon parameter. Bezier-target intersections
similarly isolate the original resultant of degree up to nine without flattening either curve.
