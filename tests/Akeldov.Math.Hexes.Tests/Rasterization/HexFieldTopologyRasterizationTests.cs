...
public class HexFieldTopologyRasterizationTests
{
    ...
    {
        var topology = new IndexSeptupletMap(new HexMapTopology(1, 1, Layout.OddR));
        var rasterizer = new HexFieldTopologyRGBA16BitRasterizer(
            new VectorXY(0f, 0f),
            2f,
            _ => new RGBA16BitColor(1, 2, 3, 4));

        SpatialRasterGrid grid = rasterizer.CreateGrid(topology, 3f);
        SpatialRaster<RGBA16BitColor> raster = rasterizer.Rasterize(topology, grid);
    ...
    {
        var topology = new IndexSeptupletMap(new HexMapTopology(2, 1, Layout.OddR));
        ...
        SpatialRasterGrid grid = rasterizer.CreateGrid(topology, 4f);
